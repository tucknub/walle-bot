...
def format_prop_embed(prop: dict, pick_id: int = None, username: str = None) -> discord.Embed:
    ...
    title = f"{grade_label} — {player} ({team}){vs_str} · {direction} {line} {stat}"
    ...
    embed.add_field(
        name="💰 Edge",
        value=f"**EV:** {ev_sign}{round(ev, 1) if ev is not None else 'N/A'}% · **True:** {true_pct}% vs **Book:** {book_pct}%",
        inline=True
    )
    ...
    footer_parts = []
    if pick_id:
        footer_parts.append(f"Saved · ID #{pick_id}")
    if username:
        footer_parts.append(username)
    if platform and platform not in ("unknown", "other", None, ""):
...
def format_parlay_embed(parlay: dict, saved: bool = False, username: str = None) -> discord.Embed:
    ...
    overall_reason = parlay.get("overall_reasoning", "")

    embed = discord.Embed(
        title=f"{overall_grade} — {n}-Leg Parlay · #{parlay_id}",
        description=overall_reason or "Wall-E's top picks.",
    ...
    for i, leg in enumerate(legs, 1):
        ...
        vs_str = f" vs {opponent}" if opponent else ""
        ev_str = f"+{round(ev,1)}%" if ev and ev > 0 else (f"{round(ev,1)}%" if ev is not None else "")
...
def format_intelligence_embed(report: dict) -> discord.Embed:
    ...
    if elite_total > 0:
        embed.add_field(name="🔥 Elite picks", value=f"**{elite_rate}% win rate** ({elite_total} picks)", inline=True)
    if good_total > 0:
        embed.add_field(name="✅ Good picks", value=f"**{good_rate}% win rate** ({good_total} picks)", inline=True)
...
def format_pending_embed(picks: list) -> discord.Embed:
    embed = discord.Embed(
        title="⏳ Pending Picks",
        description="Use `/result` to update these picks." if picks else "No pending picks yet.",
        color=0x95A5A6,
        timestamp=datetime.now(timezone.utc)
    )
    for p in picks[:15]:
